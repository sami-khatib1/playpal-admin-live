...
async function loadCandidates(page) {
    ...
    try {
        const url = `${getApiBaseUrl()}/admin/growth/group-push/candidates?${buildCandidatesQuery(page)}`;
...
async function sendGroupDiscoveryPush() {
    ...
    if (!communityId) {
        showError("Enter the community ID.");
        return;
    }
    ...
    try {
        const url = `${getApiBaseUrl()}/admin/growth/group-push`;
        const response = await fetch(url, {
            method: "POST",
            headers: {
                Authorization: `Bearer ${token}`,
        ...
        const groupName = data.community?.name ? ` → "${data.community.name}"` : "";
        const msg = `Sent: ${s.deliveredToUser ?? 0} · No device: ${s.noDeviceTokens ?? 0} · Failed: ${s.failed ?? 0}${groupName}`;
        showSuccess(msg);
        if (resultEl) {
...
window.initGroupDiscoveryPush = initGroupDiscoveryPush;
